...
namespace Exercise7
{
    public class LedHandlers(TrackedGpioPin redLed, TrackedGpioPin blueLed)
    {
        ...
        public void HandleTurnLEDsOn(TurnLEDsOn message, SenderLink sender)
        {
            redLed.Write(message.ShouldRedBeOn ? PinValue.High : PinValue.Low);
            blueLed.Write(message.ShouldBlueBeOn ? PinValue.High : PinValue.Low);
